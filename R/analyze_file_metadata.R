...
#' @param metadata_dir The path of the directory of the metadata files, if omitted,
#' will be \code{link{base::tempdir()}}.
#' @param save_file Defaults to \code{TRUE} and saves a temporary metadata file.
...
analyze_file_metadata <- function (
  ...
  tryCatch({
    ...
    warning = function(cond) {
      ...
      if ( save_file) saveRDS( metadata, paste0(tempdir(), "\\", metadata_backup ))
    }
    ) #end of tryCatch
  return ( metadata )
}
